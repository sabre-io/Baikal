...
class System extends \Flake\Core\Controller
{
    ...
    {
        if ($oForm->submitted()) {
            $bMySQL = (1 === intval($oForm->postValue('PROJECT_DB_MYSQL')));
        } else {
    ...
    public function validationHook(\Formal\Form $oForm, \Formal\Form\Morphology $oMorpho)
    {
        ...
        if (1 === intval($oForm->modelInstance()->get('PROJECT_DB_MYSQL'))) {
            ...
            if (true !== ($aMissingTables = \Baikal\Core\Tools::isDBStructurallyComplete($oDB))) {
