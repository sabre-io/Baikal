...
class Topbar extends \Flake\Core\Controller
{
    ...
    public function render()
    {
        ...
        if ('default' === $sCurrentRoute) {
            $sActiveHome = 'active';
        }
        if (
            $GLOBALS['ROUTER']::getRouteForController("\BaikalAdmin\Controller\Users") === $sCurrentRoute ||
            $GLOBALS['ROUTER']::getRouteForController("\BaikalAdmin\Controller\User\Calendars") === $sCurrentRoute ||
            $GLOBALS['ROUTER']::getRouteForController("\BaikalAdmin\Controller\User\AddressBooks") === $sCurrentRoute
        ) {
            $sActiveUsers = 'active';
        }

        if ($GLOBALS['ROUTER']::getRouteForController("\BaikalAdmin\Controller\Settings\Standard") === $sCurrentRoute) {
            $sActiveSettingsStandard = 'active';
        }

        if ($GLOBALS['ROUTER']::getRouteForController("\BaikalAdmin\Controller\Settings\System") === $sCurrentRoute) {
